...
API_TOKEN="o.4voBg8AHtmI30hfPbweFUX0O365HUUGA"
...
if [ $? -eq 0 ]; then
    ...
    curl --header "Access-Token: $API_TOKEN" \
         --header 'Content-Type: application/json' \
         --data-binary '{"type": "note", "title": "Tests were successful", "body": "Tests were successful."}' \
         --request POST \
         https://api.pushbullet.com/v2/pushes
else
    # Send a notification using Pushbullet
    curl --header "Access-Token: $API_TOKEN" \
         --header 'Content-Type: application/json' \
         --data-binary '{"type": "note", "title": "Tests failed", "body": "Tests failed."}' \
         --request POST \
         https://api.pushbullet.com/v2/pushes
